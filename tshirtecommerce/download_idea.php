...
if ( isset($_GET['key']) )
{
	...
	if (isset($_GET['is_admin'])) $is_admin = $_GET['is_admin'];
	
	$dg 		= new dg();
	$cache 		= $dg->cache('admin');
	if ($is_admin == 0) $cache = $dg->cache('design');
	
	$e_params 	= explode(':', $key);
	$user_id 	= $cache->get($e_params[0]);
	
	if ($user_id == false or count($user_id) == 0) return false;
	
	$data 		= $user_id[$e_params[1]];

	if ( count($data) > 0)
	{
		$product_id = $e_params[2];
		...
		if (isset($product))
		{
			...
			{	
				...
				if (isset($_GET['type'])) $file = $_GET['type'];
				...
				if ( isset($design->area) && isset($design->params) )
				{
					...
					if ( isset($area[$position]) && isset($params[$position]) )
					{
						...
						if ($file == 'png') $zoom_inc = 0.393700787 * $sizes_cm['width'] * 300;
				...
				{	
							...
							foreach ($texts as $text)
							{
								...
								if ($text->hasChildNodes())
								{
									foreach ($text->childNodes as $tspan)
									{
										...
										if ($tspan->getAttribute('dy') > $dy) $dy = $tspan->getAttribute('dy');
					...
					$e_orders = explode(':', $order);
					if(count($e_orders) > 1) $e_order = $e_orders[0];
					else $e_order = $order;
					
					$path = $path .DS. $e_order;
					if (!file_exists($path)) mkdir($path, 0755);
						...
						{							
							...
							if (count($files))
							{
								...
								foreach($files as $file)
								{
									if(substr($dg->url(), -2) == '//')
									{
										$http_url = substr_replace($dg->url(), '', -1); // Fix can not add zip image uploaded 2016-04-16
										$string = str_replace($http_url, '', $file); 	// Fix can not add zip image uploaded 2016-04-16
									}
									else
									{
							...
							$e_keys = explode(':', $key);
							$e_key = $key;
							if(count($e_keys) > 1) $e_key = $e_keys[0];
							$zipname = $position .'-'. $e_key.'.zip';
							$zip = new ZipArchive();
