...
<html lang="en">
  <head>
	...
	.table thead>tr>th, .table tbody>tr>th, .table tfoot>tr>th, .table thead>tr>td, .table tbody>tr>td, .table tfoot>tr>td{border-top: 0px dotted #ddd;}
	</style>
  </head>
  <body>
    <div class="container">
		<div class="row form-group">
			<center><h4><strong>Download File Design</strong></h4></center>
		</div>
		<div class="row">
			...
			<?php }else{				
				...
				{
					if (isset($data['vector'])) $vectors = json_decode($data['vector']);			
					else $vectors = json_decode($data['vectors']);
					
					
					if (isset($vectors->$position))
					{
						$views = (array) $vectors->$position;
						if (count($views) == 0) $data = array();
					}
					else
					{
						$data = array();
				...
				if ( count($data) > 0)
				{
					...
					for($i = 0; $i < count($products); $i++)
					{
						...
						else if ( isset ($data['item']['product_id']) ) $product_id = $data['item']['product_id'];
						else $product_id = 0;
				...
				<div class="col-sm-6 col-md-6">
					...
					<center>Click to download: 						
						<a href="<?php echo $file.'?key='.$key.'&view='.$position.'&type=svg&download=1&is_admin='.$is_admin; ?>"><strong>SVG</strong></a>
						 or 
						<a href="javascript:void(0)" onclick="window.print();"><strong>PDF</strong></a>						
						 or 
						<a href="javascript:void(0)" onclick="downloadPNG('<?php echo $position; ?>')"><strong>PNG</strong></a>
				...
				<div class="col-sm-6 col-md-6">
					<div class="panel panel-default">
						<div class="panel-heading"><strong>Design detail</strong><br /><small class="text-danger">(Click font name to download and install font to your computer)</small></div>
						<div class="panel-body">
							<p>
								<strong>Area Design: </strong>
								<br />
								<div class="row">
									<div class="col-md-3 col-xs-3">Width: <br/><strong><?php echo number_format($sizes_cm['width'], 2); ?></strong>CM</div>
									<div class="col-md-3 col-xs-3">Height: <br/><strong><?php echo number_format($sizes_cm['height'], 2); ?></strong>CM</div>
									<div class="col-md-3 col-xs-3">Top: <br/><strong><?php echo number_format(($sizes_px['top'] * $zoom), 2); ?></strong>CM</div>
									<div class="col-md-3 col-xs-3">Left: <br/><strong><?php echo number_format(($sizes_px['left'] * $zoom), 2); ?></strong>CM</div>
								</div>
								<hr />
							</p>
							<?php 
							...
							<p>
								...
								<div class="row">
									...
									<div class="col-md-3 col-xs-3">Width: <br/><strong><?php echo number_format($print_size[$position]['width'], 2); ?></strong>CM</div>
									<?php } ?>
									
									<?php if ( isset($print_size[$position]['height']) ) { ?>
									<div class="col-md-3 col-xs-3">Height: <br/><strong><?php echo number_format($print_size[$position]['height'], 2); ?></strong>CM</div>
									<?php } ?>
									
									<?php if ( isset($print_size[$position]['size']) ) { ?>
									<div class="col-md-3 col-xs-3">Page: <br/><strong>A<?php echo $print_size[$position]['size']; ?></strong></div>
									<?php } ?>
								</div>
								...
								<p>									
									<?php foreach($print_colors[$position] as $p_color) { ?>
										<button type="button" data-toggle="tooltip" data-placement="top" title="<?php echo $p_color; ?>" style="background-color:#<?php echo $p_color; ?>;padding:15px;margin-bottom:4px;" class="btn btn-default"></button>
									<?php } ?>
								</p>
								<?php } ?>
								<hr />
								...
								foreach($items as $item)
								{
									...
									if (isset($item['colors']) && is_array($item['colors']) && count($item['colors']) > 0)
									{
										...
										foreach($colors as $itemColor)
										{
											...
											echo '<button type="button" data-toggle="tooltip" data-placement="top" title="'
												.$itemColor.'" style="background-color:#'.$itemColor.';padding:15px;margin-bottom:4px;" class="btn btn-default"></button>';
										}
										echo '</p>';
									}
									echo '</div>';
									...
									echo 	'<tr class="tr-first">';
									echo 		'<td>Width:</td>';
									echo 		'<td><strong>'.number_format(($item['width'] * $zoom), 2).'CM</strong></td>';
									echo 	'</tr>';
									echo 	'<tr>';
									echo 		'<td>Height:</td>';
									echo 		'<td><strong>'.number_format(($item['height'] * $zoom), 2).'CM</strong></td>';
									echo 	'</tr>';
									echo 	'<tr>';
									echo 		'<td>Top:</td>';
									echo 		'<td><strong>'.number_format(($item['top'] * $zoom), 2).'CM</strong></td>';
									echo 	'</tr>';
									echo 	'<tr>';
									echo 		'<td>Left:</td>';
									echo 		'<td><strong>'.number_format(($item['left'] * $zoom), 2).'CM</strong></td>';
									echo 	'</tr>';
									if (empty($item['rotate'])) $item['rotate'] = 0;
									echo 	'<tr>';
									echo 		'<td>Rotate:</td>';
									echo 		'<td><strong>'.$item['rotate'].'</strong></td>';
									echo 	'</tr>';
									echo '</table>';
									echo '</div>';
									echo '</div>';
	...
	<span id="download-pdf" style="display:none;"><?php echo $download_pdf; ?></span>
