...
var design={
	...
	ini:function(){
		...
		$jd('.add_item_admindesign').click(function(){
			self.ajax.admindesign();
		});
		...
		admindesign: function(e){
			...
			{				
				...
				jQuery.ajax({
					...
					page = page + 1;
					if (jQuery('#dg-admindesign img').length > (page*8)-1)
					{
	...
	saveDesignAdmin: function(){		
		...
		jQuery.ajax({
			url: siteURL + "ajax.php?type=saveDesignAdmin",
			type: "POST",
			contentType: 'application/json',
			data: JSON.stringify(data),
		}).done(function( msg ) {
	...
	saveInfo: function(e){
		...
		jQuery.ajax({
			...
			url: siteURL.replace('/tshirtecommerce/', '/index.php?route=tshirtecommerce/designer_api/valid_customer'),
			type: "POST",			
