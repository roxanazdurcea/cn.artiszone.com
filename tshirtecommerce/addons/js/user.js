...
design.user = {
	ini: function(e, type)
	{
		...
		else if(type == 'register')
		{
			...
			var url = mainURL + 'index.php?route=tshirtecommerce/designer_api/register';
		}
		else
		{
			return false;
		...
		jQuery.ajax({
			...
			{	
				...
				if(valid_login == -1)
				{
					...
					jQuery('#'+type+'-status-exist').css('display', 'none');
				...
				{
					jQuery('#'+type+'-status-exist').css('display', 'block');
					jQuery('#'+type+'-status').css('display', 'none');
					jQuery('#'+type+'-status a').click(function(e){
						e.preventDefault(); 
						var url = jQuery(this).attr('href'); 
