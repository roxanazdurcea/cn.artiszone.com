...
switch($type){
	...
	case 'removeDesign':
		...
		break;
	// remove design admin
	case 'removeDesignAdmin':
		if (empty($_SESSION['is_admin']) && $_SESSION['is_admin'] === false)
		{
			echo '<h5><center>Please login!</center></h5>';
			exit;
		}
	...
	case 'adminDesign':
		...
		{	
			...
			foreach($designs as $key => $design)
			{
				...
				//?product=71&color=000000&user=c4ca4238a0b923820dcc509a6f75849b&id=1461986944864&parent=30
				$link = '?product='.$design['product_id'].'&color='.$design['product_options'].'&user='.$user.'&id='. $design['design_id'];
				...
				$html 	.=	'</a>'
						.	'<span class="design-action design-action-remove" onclick="design.ajax.removeDesignAdmin(this)" data-id="'.$user.':'.$key.'" title="Remove this design"><i class="red glyphicons remove_2"></i></span>'
						. '</div>';
			}
			echo $html;
		}
	...
	case 'userDesign':
		...
		// get all design
		if ($is_logged['is_admin'] === true && (!isset($is_logged['is_customer']) || $is_logged['is_customer'] === false))
		{			
			$cache = $dg->cache('admin');
		}
		else
